...
public class OneDayService implements IDailyForecastService {
    @Override
    ...
    @Override
    public OneDayForecast getWeather(String cityName) throws IOException {
        String locationKey = getLocationKey(cityName);
        String urlString = "https://dataservice.accuweather.com/forecasts/v1/daily/1day/"
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                Gson gson = new Gson();
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getInputStream()))) {
                    OneDayForecast responseObj = gson.fromJson(reader, OneDayForecast.class);
                    return responseObj;
                }
